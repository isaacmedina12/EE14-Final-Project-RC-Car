...
// Variable to hold the amount of time the SysTick interupt has triggered
volatile uint32_t SysTick_counter = 0;
void  SysTick_Handler(void){
    SysTick_counter++; // Always update for display refresh
}
...
void SysTick_initialize(void){
    ...
    // Set to 3999 for 1kHz,  or 1ms interval
    // Formula: (4MHz / 1kHz) - 1
    SysTick->LOAD = 3999; 
    ...
    // Use the system base clock
    SysTick->CTRL |= SysTick_CTRL_CLKSOURCE_Msk;

    // Enable the interupt signal
...
// delay_ms()
// Input: A 32-bit unsigned integer for a delay time in ms
// Description:
// Output:
void delay_ms(uint32_t delay_milis){

    // Compute the target time
    uint32_t target_time = SysTick_counter + delay_milis;

    // Wait until the target time is reached
    while(SysTick_counter < target_time);
}